...
- אתה לא יכול להקצות, לזרוק, או להשתמש ב-`new`. `{{ x = 5 }}` לא יעבור כביטוי תבנית.
...
בקריאה מימין לשמאל, הסוגריים בתוך הסוגריים המרובעים הם עזר הזיכרון של "בננה בקופסה". התחביר מתרחב ל-property binding על `ngModel` ו-event binding על `ngModelChange`:
