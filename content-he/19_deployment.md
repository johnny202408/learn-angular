...
אם Compass מרונדר בלקוח (ללא SSR), כל אירוח סטטי עובד. המכניקה שונה; העיקרון זהה: העלה את `dist/compass/browser/` והגדר את המארח להפנות ל-`index.html` עבור מסלולים לא תואמים (fallback).
