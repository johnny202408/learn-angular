...
Compass היה מסך יחיד. יישומים אמיתיים יש להם כמה — בית, הגדרות, תצוגת פירוט, לפעמים עשרות. ה-*ראוטר* של Angular הוא החלק שהופך URL אחד לעץ קומפוננטות אחד, מעדכן אותו כשהמשתמש מנווט, ומאפשר לחלקים של האפליקציה להגיב לאיזה URL פעיל.
...
צורת המערך של `routerLink` בונה את ה-URL ממקטעים. הוא מטפל בקידוד URL בשבילך. העדף אותו על פני `'/task/' + task().id`.
...
**`canMatch`** — האם המסלול הזה בכלל צריך להיחשב להתאמה? נמצא בשימוש כדי להסתיר מסלולים לגמרי ממשתמשים שלא צריכים להגיע אליהם. טוב יותר מ-`canActivate` לאימות כי הקוד של המסלול לא מורד אם `canMatch` מחזיר false.
