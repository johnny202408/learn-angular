...
- **סיגנל פרטי לכתיבה, תצוגת readonly ציבורית.** `_tasks` הוא פרטי וניתן לשינוי; `tasks` הוא הסיגנל הציבורי, לקריאה בלבד שצרכנים רואים. זה מונע מצרכנים לעשות `this.store.tasks.set(...)` ולעקוף את ה-API.
...
2. הוסף שירות `HabitStore` שנבנה באותה דרך כמו `TaskStore`. הוא צריך להחזיק סיגנל של הרגלים (המצא `interface Habit`), לחשוף `habits` לקריאה בלבד, ולספק `add`, `remove`, ו-`check`. אל תחבר אותו עדיין לאף קומפוננטה; רק תוודא שהוא מתקמפל.
