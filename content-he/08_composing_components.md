...
הנה איפה ש-Compass ייגמר, מבנית, אחרי הפרק הזה וכמה הבאים:
...
כל תיבה היא קומפוננטה. חיצים מלאים הם יחסי *מכיל* (ההורה מרנדר את הילד בתבנית שלו). כל קומפוננטה עצמאית; לכל אחת יש קובץ משלה. זו הצורה של אפליקציית Angular.
...
החלק השלישי מתחיל בפרק 9 על *הזרקת תלויות* — המנגנון שיאפשר לנו למשוך את נתוני המשימות מ-`TaskList` לתוך שירות שקומפוננטות אחרות יכולות לשתף. פרק 10 מחבר את השירות הזה לבקאנד HTTP; פרק 11 מציג RxJS לחלקים של האפליקציה שבהם סיגנלים לא מספיקים; פרק 12 מחליף את הטופס שנעשה ידנית בטפסים ריאקטיביים.

עד סוף החלק השלישי, Compass ידבר עם שרת אמיתי, ישמור את המשימות שאתה מוסיף, ויפסיק לאבד את העבודה שלך כשאתה מרענן.
