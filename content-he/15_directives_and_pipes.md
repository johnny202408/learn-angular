...
לקבל את זה נכון חוסך הרבה קוד. עטיפת `<div>` ב-`<app-tooltip-host>` כשדירקטיבת `[appTooltip]` הייתה מספיקה מכפילה את האלמנטים ב-DOM. פייפ של חישוב כשסיגנל `computed` בבעלות קומפוננטה היה מספיק מפצל את הלוגיקה בכל הקוד.
